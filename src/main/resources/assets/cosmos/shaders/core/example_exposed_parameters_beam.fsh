...
void main() {
    vec3 node_color_1778595630929 = u_color_2;
    vec3 node_color_1778595628747 = u_color_1;
    vec2 node_uv = vUv;
    ...
    float node_nx = node_split_x * 6.0;
    ...
    float node_fbm = atomic_fbm3D(node_n_pack, int(4.0));
    float node_f_sharp = pow(node_fbm, 2.0);
    float node_f_boost = node_f_sharp * 3.0;
    vec3 node_mix_base = mix(node_color_1778595630929, node_color_1778595628747, clamp(node_f_boost, 0.0, 1.0));
